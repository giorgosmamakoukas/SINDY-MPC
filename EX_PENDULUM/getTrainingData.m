...
% Parameters: Model
p.G = 9.81;             % True system parameters
p.M = 28;
p.L = 8/3;
n = 2;
x0=[pi; 0];  % Initial condition
...
switch InputSignalType
    case 'sine2'
        ...
        [t,x]=ode45(@(t,x) PendulumSys(t,x,forcing(x,t),p),tspan,x0,options);
        u = forcing(0,tspan);
    ...
    case 'noise'
        vareps = 0.01; 
        Diff = @(t,x) [0; vareps];
        SDE = sde(@(t,x) PendulumSys(t,x,forcing(x,t),p),Diff,'StartState',x0');
        rng(1,'twister')
    ...
    case 'mixed'
        ...
        [t,x] = ode45(@(t,x) PendulumSys(t,x,forcing(x,t),p),tspan1,x0,options);
        ...
        [tv,xv]=ode45(@(t,x) PendulumSys(t,x,forcing(x,t),p),tspanv,x(end,:),options);
